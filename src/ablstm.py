...
class LstmModule(nn.Module):
    def __init__(self, input_units, output_units, hidden_units, batch_size, bias = True, num_chunks = 2, embedding_dim = 50):
        ...
        self.i = nn.Parameter(torch.Tensor(batch_size, hidden_size))
        self.f = nn.Parameter(torch.Tensor(batch_size, hidden_size))
        if bias:
    ...
    def forward(self, input_, hx = None):
        ...
        o = self.sigmoid(w_w[0 : self.hidden_size]).transpose(0, 1)
        g = self.tanh(w_w[self.hidden_size : 2*self.hidden_size]).transpose(0, 1)

        c = (self.f * cprev) + (self.i * g)
        h = o * self.relu(c)

        return (h, c), o, cprev, g
