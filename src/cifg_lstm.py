...
class LstmModule(nn.Module):
    def __init__(self, input_units, output_units, hidden_units, batch_size, bias = True, num_chunks = 3, embedding_dim = 50):
    ...
    def forward(self, input_, hx = None):
        ...
        o = self.sigmoid(w_w[self.hidden_size : 2*self.hidden_size]).transpose(0, 1)
        g = self.tanh(w_w[2*self.hidden_size : 3*self.hidden_size]).transpose(0, 1)
        f = 1.0 - i
...
class LSTM(nn.Module):
    ...
    def forward(self, input_, max_time = 50) :
        ...
        allh = torch.Tensor(all_hidden)
        x, y, z = allh.size()
        allh = allh.view(y, x, z)
        allo = torch.Tensor(all_outputs)
        x, y, z = allo.size()
        allo = allo.view(y, x, z)
        all_hidden = allh.tolist()
        # print(all_outputs[49])
